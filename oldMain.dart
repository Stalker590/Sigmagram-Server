...
void main() async {
  ...
  final password = "1234";
  final text_for_send = "Hello World!";
  final send_id_of_chat = "555324";
  final isAuth = await isAuthenticated(userName, password, userUseCase);
  final action = "login";

  if (isAuth.isAuthed) {
    print("Login success: ${isAuth.user?.id}");
    switch(action){
      case "send_messenge":
        if (isAuth.isAuthed && isAuth.user != null) {
          final user = isAuth.user!; 
          switch (action) {
            case "send_messenge":
              await messengesUseCase.SendMessenge(text_for_send, user.id, send_id_of_chat);
              break;
          }
      }
    ...
    print("Login failed: ${isAuth.user?.id}");
  }
